...
(function () {
    ...
    function MT_installHook() {
        ...
        RendererStrategy.render = function (rendererObj, stage) {
            ...
            if (!_MT_loggedOnce) {
                _MT_loggedOnce = true;
                console.log('[MenuTransition] === 씬위에 씬 렌더 시작 ===');
                console.log('[MenuTransition] SceneManager._stack.length:', stack ? stack.length : 'N/A');
                console.log('[MenuTransition] prevScene:', prevScene ? prevScene.constructor.name : 'null (폴백: 현재 씬 사용)');
                console.log('[MenuTransition] currentScene (stage):', stage ? stage.constructor.name : 'null');
                console.log('[MenuTransition] bgStage === prevScene?', bgStage === prevScene);
                console.log('[MenuTransition] bgStage._backgroundSprite:', bgStage && bgStage._backgroundSprite ? '있음' : '없음');
                console.log('[MenuTransition] Cfg:', JSON.stringify(Cfg));
                console.log('[MenuTransition] captureRT size:', w + 'x' + h);
            }
