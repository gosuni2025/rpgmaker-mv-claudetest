...
(function() {
    ...
    console.log('[OcclusionSilhouette] 플러그인 로드 시작',
        'PostProcess:', typeof PostProcess,
        'MapRenderPass:', typeof MapRenderPass,
        'Simple2DRenderPass:', typeof Simple2DRenderPass,
    ...
    PostProcess._createComposer = function(rendererObj, stage) {
        ...
        OcclusionSilhouette._logged3D = false;
        console.log('[OcclusionSilhouette] 3D Composer hook - SilhouettePass 삽입됨, passes:', passes.length);
    };
    ...
    PostProcess._createComposer2D = function(rendererObj, stage) {
        ...
        OcclusionSilhouette._silhouettePass = silPass;
        OcclusionSilhouette._logged2D = false;
        console.log('[OcclusionSilhouette] 2D Composer hook - SilhouettePass 삽입됨, passes:', passes.length);
    };
    ...
    MapRenderPass.prototype.render = function(renderer, writeBuffer, readBuffer, deltaTime, maskActive) {
        ...
        if (OcclusionSilhouette._active) {
            ...
            if (!OcclusionSilhouette._logged3D) {
                OcclusionSilhouette._logged3D = true;
                var sp = this.spriteset;
                console.log('[OcclusionSilhouette] 3D Hook 실행됨',
                    'spriteset:', !!sp,
                    'charSprites:', sp && sp._characterSprites ? sp._characterSprites.length : 0,
                    'objSprites:', sp && sp._objectSprites ? sp._objectSprites.length : 0,
                    'hasMasks:', hasMasks,
                    'silhouettePass:', !!OcclusionSilhouette._silhouettePass);
                if (sp && sp._objectSprites) {
                    for (var di = 0; di < sp._objectSprites.length; di++) {
                        var ds = sp._objectSprites[di];
                        console.log('  obj[' + di + ']:', ds._mapObjName, 'visible:', ds.visible, '_threeObj:', !!ds._threeObj, '_threeObj.visible:', ds._threeObj ? ds._threeObj.visible : 'N/A');
                    }
                }
                if (sp && sp._characterSprites) {
                    var playerSprites = OcclusionSilhouette._getPlayerSprites(sp);
                    console.log('  playerSprites found:', playerSprites.length);
                    for (var pi = 0; pi < playerSprites.length; pi++) {
                        var ps = playerSprites[pi];
                        console.log('    player[' + pi + ']:', ps._character === $gamePlayer ? 'Player' : 'Follower', '_threeObj:', !!ps._threeObj);
                    }
                }
            }
        }
    ...
    Simple2DRenderPass.prototype.render = function(renderer, writeBuffer, readBuffer, deltaTime, maskActive) {
        ...
        if (OcclusionSilhouette._active) {
            ...
            if (rendererObj) {
                ...
                if (!OcclusionSilhouette._logged2D) {
                    OcclusionSilhouette._logged2D = true;
                    var sp2d = Mode3D._spriteset;
                    console.log('[OcclusionSilhouette] 2D Hook 실행됨',
                        'spriteset:', !!sp2d,
                        'charSprites:', sp2d && sp2d._characterSprites ? sp2d._characterSprites.length : 0,
                        'objSprites:', sp2d && sp2d._objectSprites ? sp2d._objectSprites.length : 0,
                        'hasMasks:', hasMasks);
                }
            }
        }
